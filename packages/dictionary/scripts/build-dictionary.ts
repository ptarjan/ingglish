...
async function main(): Promise<void> {
  ...
  const jsonContent = JSON.stringify(dict);

  // JSON file for Node.js fast path (readFileSync + JSON.parse bypasses esbuild)
  const JSON_OUTPUT_PATH = OUTPUT_PATH.replace('.js', '.json');
  await fs.writeFile(JSON_OUTPUT_PATH, jsonContent);
  console.log(`Written to ${JSON_OUTPUT_PATH}`);

  // JS file with JSON.parse() wrapper for Vite/browser (V8 parses JSON ~2x
  // faster than equivalent JS object literals). Velar nasals pre-normalized above.
  const jsContent = `// Auto-generated CMU dictionary - do not edit
// Generated by scripts/build-dictionary.ts
const cmudict = JSON.parse(\`${jsonContent}\`);
...
async function generateWordFrequencies(): Promise<void> {
  ...
  const jsonContent = JSON.stringify(frequencyMap);

  // JSON for Node.js fast path
  await fs.writeFile(WORD_FREQ_OUTPUT_PATH.replace('.js', '.json'), jsonContent);

  // JS with JSON.parse wrapper for Vite/browser
  const jsContent = `// Auto-generated word frequencies - do not edit
// Generated from subtlex-word-frequencies npm package
const WORD_FREQUENCIES = JSON.parse(\`${jsonContent}\`);
...
async function generateReverseDictionary(dict: CMUDictionary): Promise<void> {
  ...
  const jsonContent = JSON.stringify(reverseDict);

  // JSON for Node.js fast path
  await fs.writeFile(REVERSE_OUTPUT_PATH.replace('.js', '.json'), jsonContent);
...
const reverseCmudict = JSON.parse(\`${jsonContent}\`);
