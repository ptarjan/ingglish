 ...
 * Ranked by frequency-weighted impact.
 *
 * Usage: npx tsx --conditions=source packages/core/scripts/g2p/rule-errors.ts
 */
...
export async function main() {
  ...
  console.log('=== Top 50 Rule-Level Errors (by freq weight) ===\n');
  const byFreq = Object.entries(ruleErrors).sort((a, b) => b[1].freqSum - a[1].freqSum);
  for (const [key, { count, freqSum, examples }] of byFreq.slice(0, 50)) {
    console.log(
  ...
  console.log('\n=== Top 50 Rule-Level Errors (by word count) ===\n');
  const byCount = Object.entries(ruleErrors).sort((a, b) => b[1].count - a[1].count);
  for (const [key, { count, freqSum, examples }] of byCount.slice(0, 50)) {
    console.log(
...
if (process.argv[1]?.includes('rule-errors')) main().catch(console.error);
