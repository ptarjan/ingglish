...
export async function main() {
  ...
  const wordsByScore = allWords
    ...
    .map((w) => ({
      word: w,
      spelling: baselineSpellings.get(w)!,
      score: baselineWordScores.get(w)!,
    }))
  ...
  for (let si = 0; si < sortedStress0.length; si++) {
    ...
    for (const option of allOptions) {
      ...
      if (
        hasSpellingConflict(rawPhoneme, option, ARPABET_TO_INGGLISH_MAP, baselineStressOverrides)
      ) {
    ...
    for (const { key, from, to, delta } of stressImprovements.slice(0, 5)) {
      ...
      const wordDeltas = affected
        .map((word) => {
          ...
          const freq = getWordFrequency(word) ?? 0;
          const oldSpelling = baselineSpellings.get(word)!;
          const newSpelling = phonemesToSpelling(
            rawPhonemes,
            ARPABET_TO_INGGLISH_MAP,
            testOverrides
          );
  ...
  for (const imp of appliedChanges) {
    ...
    const examples = affected
      ...
      .map((w) => {
        ...
        const oldSpelling = phonemesToSpelling(
          raw,
          ARPABET_TO_INGGLISH_MAP,
          baselineStressOverrides
        );
...
if (process.argv[1]?.includes('g2p-roundtrip-search')) main().catch(console.error);
