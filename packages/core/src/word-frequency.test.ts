...
import { getWordFrequency, scoreWord, sortByFrequency } from './word-frequency';
...
describe('word-frequency', () => {
  ...
  describe('scoreWord', () => {
    it('should give lower scores to more common words', () => {
      // "the" should score lower (better) than "xylophone"
  ...
  describe('sortByFrequency', () => {
    it('should sort words by frequency (most common first)', () => {
      ...
      expect(sorted[0]).toBe('the'); // "the" is most common
    });
