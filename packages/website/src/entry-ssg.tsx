...
// Docs renders <title>, and AppLayout and Docs render <meta name="description">
// and <link rel="canonical">, into the tree because React 19 hoists them into
// <head> in the browser. renderToString does not hoist, so on the server they
// stay where they were rendered — inside #root — and the page ends up with a
// second <title> and two or three conflicting descriptions, none of them in
// <head>. The <head> copies written by customizeHtml in vite.config.ts are the
// only ones that count, so drop these. React writes attributes in its own
// order, so match on the identifying attribute rather than on the start of the
// tag; <title> has no attribute to key on, so match the whole element instead.
const HEAD_ONLY_TAGS =
  /<title>[^<]*<\/title>|<meta [^>]*name="description"[^>]*>|<link [^>]*rel="canonical"[^>]*>/g;
