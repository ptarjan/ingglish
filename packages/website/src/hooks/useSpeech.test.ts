...
describe('useSpeech', () => {
  ...
  it('highlights words on a timer when no boundary events fire (non-English voices)', () => {
    const { result } = renderHook(() => useSpeech()) as SpeechHook;

    // Speak French text — most non-English voices never fire onboundary
    act(() => {
      result.current[1]('Bonjour le monde', 'fr');
    });

    // No boundary events fired — wordCount should still be null initially
    expect(result.current[4]).toBeNull();

    // After the fallback delay kicks in, it should start advancing words
    act(() => {
      vi.advanceTimersByTime(500);
    });
    expect(result.current[4]).toBe(0);

    // Should advance to the next word after the per-word interval
    act(() => {
  ...
  it('does not use timer fallback when boundary events fire', () => {
    const { result } = renderHook(() => useSpeech()) as SpeechHook;
    ...
    // Fire a real boundary event before the fallback timer
    act(() => {
    ...
    // After fallback delay, should NOT override — still at word 0
    act(() => {
      vi.advanceTimersByTime(500);
    });
    expect(result.current[4]).toBe(0);
  ...
  it('stops timer fallback when speech ends', () => {
    const { result } = renderHook(() => useSpeech()) as SpeechHook;

    act(() => {
      result.current[1]('un deux trois', 'fr');
    });
    ...
    // Timer should not advance after end
    act(() => {
